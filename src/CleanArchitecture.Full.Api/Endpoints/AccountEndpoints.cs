...
public static class AccountEndpoints
{
    ...
    {
        var group = app.MapGroup("v1/api/accounts").WithTags("Accounts (Minimal API)");
        ...
        group.MapPost("", async (CreateAccountCommand command, ISender sender, CancellationToken cancellationToken) =>
        {
            ...
            return Results.Created($"v1/api/accounts/{account.Id}", account);
        })
        ...
        {
            var account = await sender.Send(new UpdateAccountCommand(id, body.AccountNumber, body.HolderName, body.Balance, body.Status, body.AccountType), cancellationToken);
            return account is null ? Results.NotFound() : Results.Ok(account);
        })
            .Produces<AccountDto>(StatusCodes.Status200OK)
            .Produces(StatusCodes.Status400BadRequest)
...
public record UpdateAccountBody(string AccountNumber, string HolderName, decimal Balance, string Status, string AccountType);
